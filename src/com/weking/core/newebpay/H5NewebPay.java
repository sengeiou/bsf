...
public class H5NewebPay {
    private static Logger log = Logger.getLogger(H5NewebPay.class);
    ...
    static {
        key = WKCache.get_system_cache(C.WKSystemCacheField.pay_H5NewebPay_key);
        iv = WKCache.get_system_cache(C.WKSystemCacheField.pay_H5NewebPay_iv);
        returnUrl = WKCache.get_system_cache(C.WKSystemCacheField.pay_H5NewebPay_h5);
        MerchantID = WKCache.get_system_cache(C.WKSystemCacheField.pay_H5NewebPay_MerchantID);
        email = WKCache.get_system_cache(C.WKSystemCacheField.pay_H5NewebPay_email);
    }

    public static JSONObject getPostData(String orderSn,int amount) {
        //log.info("非官网加密文件==========="+orderSn+amount);
        try {
          //  log.info("非官网数据库获取==========="+key+"iv:"+iv+"returnUrl:"+returnUrl+"MerchantID:"+MerchantID+"email:"+email);
            ...
            //log.error("非官网AES加密前==========="+object.toString());
            String TradeInfo = Encrypt(str);//获得 AES 加密
            //log.error("非官网AES加密==========="+TradeInfo);
